...
import cn.niit.springbootexceptionhandler.entity.EmployeeEx;
import cn.niit.springbootexceptionhandler.util.EmployeeExJsonException;
import cn.niit.springbootexceptionhandler.util.EmployeeExNotFoundException;
import cn.niit.springbootexceptionhandler.util.NotAcceptedException;
...
public class EmployeeExController {
	...
	public String getEmployee(@PathVariable("id") int id, Model model) throws Exception {
		...
		if (id == 1) {
			...
			throw new IOException("IOException异常，数据库没有用户id=" + id);
		} else if (id == 4) {
